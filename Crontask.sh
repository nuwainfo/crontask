...
main() {
  ...
  f_stdout=$logdir$PROGNAME.$cmduniq.out.txt
  f_stderr=$logdir$PROGNAME.$cmduniq.err.txt
  f_timing=$logdir$PROGNAME.$cmduniq.tim.txt
  ...
  if $progtime -p bash -c "$runcmd1 $runcmd2 $command 1>> $f_stdout 2>> $f_stderr" 2>> $f_timing ; then
    # program executed ok
    echo "program executed ok"
    if [[ -n "$hchk" ]] && [[ ! "$hchk" == " " ]]; then
      # ping hchk.io
      webhook="https://healthchecks.nuwainfo.com/ping/$hchk"
    ...
    if [[ -n "$hchk" ]] && [[ ! "$hchk" == " " ]]; then
      # ping hchk.io
      webhook="https://healthchecks.nuwainfo.com/ping/$hchk/fail"
      log "program success: calling $webhook"
      if curl -s $webhook > /dev/null ; then
        log "Call to [$webhook]: OK"
      else
